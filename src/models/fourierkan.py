...
class FourierKAN(torch.nn.Module):
    def __init__(self, layer_sizes, gridsize, addbias=True, smooth_initialization=False):
        """
        layer_sizes: list of integers like [n, m, k, ...]
                     Each consecutive pair defines a layer.
        gridsize: number of Fourier coefficients per input feature.
        """
...
def get_fourierkan_model(input_size, **kwargs):
    
    
    return FourierKAN(
        layer_sizes=kwargs["layer_sizes"],
        gridsize=kwargs["gridsize"],
        addbias=kwargs["addbias"],
        smooth_initialization=kwargs["smooth_initialization"] )       
